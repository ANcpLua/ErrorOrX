...
public interface ITodoService
{
    ...
    Task<ErrorOr<List<Todo>>> GetAllAsync(CancellationToken ct);
    ...
    Task<ErrorOr<Todo>> CreateAsync(CreateTodoRequest request, CancellationToken ct);

    /// <summary>Update a todo. Returns NotFound if not exists.</summary>
    [ReturnsError(ErrorType.NotFound, "Todo.NotFound")]
    Task<ErrorOr<Updated>> UpdateAsync(Guid id, UpdateTodoRequest request, CancellationToken ct);

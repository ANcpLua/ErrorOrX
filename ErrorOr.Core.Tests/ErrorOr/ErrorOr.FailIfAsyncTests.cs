...
public class FailIfAsyncTests
{
    ...
    public async Task CallingFailIfAsyncWithErrorBuilder_WhenFailsIf_ShouldReturnError()
    {
        ...
        // Act
        ErrorOr<int> result = await errorOrInt
            .FailIfAsync(static num => Task.FromResult(num > 3), static num => Task.FromResult(Error.Failure(description: $"{num} is greater than 3.")));
    ...
    public async Task CallingFailIfAsyncWithErrorBuilder_WhenDoesNotFailIf_ShouldReturnValue()
    {
        ...
        // Act
        ErrorOr<int> result = await errorOrInt
            .FailIfAsync(static num => Task.FromResult(num > 10), static num => Task.FromResult(Error.Failure(description: $"{num} is greater than 10.")));

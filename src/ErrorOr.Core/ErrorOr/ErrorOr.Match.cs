...
public readonly partial record struct ErrorOr<TValue> : IErrorOr<TValue>
{
    ...
    {
        if (IsError) return await onError(Errors).ConfigureAwait(false);
    ...
    {
        if (IsError) return onFirstError(FirstError);
    ...
    {
        if (IsError) return await onFirstError(FirstError).ConfigureAwait(false);

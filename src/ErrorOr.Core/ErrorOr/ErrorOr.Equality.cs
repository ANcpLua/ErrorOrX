...
public readonly partial record struct ErrorOr<TValue>
{
    ...
    {
        if (!IsError) return !other.IsError && EqualityComparer<TValue>.Default.Equals(_value, other._value);
    ...
    {
        if (!IsError) return _value.GetHashCode();

        var hashCode = new HashCode();
        for (var i = 0; i < _errors.Count; i++) hashCode.Add(_errors[i]);
    ...
    private static bool CheckIfErrorsAreEqual(IReadOnlyList<Error> errors1, IReadOnlyList<Error> errors2)
    {
        ...
        if (ReferenceEquals(errors1, errors2)) return true;

        if (errors1.Count != errors2.Count) return false;

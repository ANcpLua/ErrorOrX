...
public sealed partial class ErrorOrEndpointGenerator
{
        ...
        ImmutableArray<DiagnosticInfo>.Builder diagnostics)
    {
        ...
        {
            diagnostics.Add(DiagnosticInfo.Create(
                DiagnosticDescriptors.InvalidFromRouteType,
                method,
                meta.Name,
                meta.TypeFqn));
        ...
        ImmutableArray<DiagnosticInfo>.Builder diagnostics)
    {
        ...
        diagnostics.Add(DiagnosticInfo.Create(
            DiagnosticDescriptors.InvalidFromQueryType,
            method,
            meta.Name,
            meta.TypeFqn));
        return ParameterClassificationResult.Error;
        ...
        ImmutableArray<DiagnosticInfo>.Builder diagnostics)
    {
        ...
        diagnostics.Add(DiagnosticInfo.Create(
            DiagnosticDescriptors.InvalidFromHeaderType,
            method,
            meta.Name,
            meta.TypeFqn));
        return ParameterClassificationResult.Error;
        ...
        ErrorOrContext context)
    {
        ...
        {
            diagnostics.Add(DiagnosticInfo.Create(
                DiagnosticDescriptors.InvalidAsParametersType,
                method,
                meta.Name,
                meta.TypeFqn));
            return ParameterClassificationResult.Error;
        ...
        {
            diagnostics.Add(DiagnosticInfo.Create(
                DiagnosticDescriptors.AsParametersNoConstructor,
                method,
    ...
    private static RoutePrimitiveKind? TryGetRoutePrimitiveKindByFqn(ITypeSymbol type)
    {
        ...
        return fqn switch
        {
            WellKnownTypes.Fqn.Guid or "global::System.Guid" or "System.Guid" => RoutePrimitiveKind.Guid,
            WellKnownTypes.Fqn.DateTime or "global::System.DateTime" or "System.DateTime" => RoutePrimitiveKind
                .DateTime,
            WellKnownTypes.Fqn.DateTimeOffset or "global::System.DateTimeOffset" or "System.DateTimeOffset" =>
                RoutePrimitiveKind.DateTimeOffset,
            WellKnownTypes.Fqn.DateOnly or "global::System.DateOnly" or "System.DateOnly" => RoutePrimitiveKind
                .DateOnly,
            WellKnownTypes.Fqn.TimeOnly or "global::System.TimeOnly" or "System.TimeOnly" => RoutePrimitiveKind
                .TimeOnly,
            WellKnownTypes.Fqn.TimeSpan or "global::System.TimeSpan" or "System.TimeSpan" => RoutePrimitiveKind
                .TimeSpan,
            _ => null
        };
    }
    ...
    private static CustomBindingMethod DetectBindAsyncMethod(INamedTypeSymbol type, ErrorOrContext context)
    {
        foreach (var member in type.GetMembers("BindAsync"))
        {
            ...
            var firstParamFqn = method.Parameters[0].Type.ToDisplayString();
            if (firstParamFqn != "Microsoft.AspNetCore.Http.HttpContext" &&
                firstParamFqn != "global::Microsoft.AspNetCore.Http.HttpContext")
                continue;

            // Check for ParameterInfo overload
            if (method.Parameters.Length >= 2)
            {
                var secondParamFqn = method.Parameters[1].Type.ToDisplayString();
                if (secondParamFqn == "System.Reflection.ParameterInfo" ||
                    secondParamFqn == "global::System.Reflection.ParameterInfo" ||
                    (context.ParameterInfo is not null &&
                     SymbolEqualityComparer.Default.Equals(method.Parameters[1].Type, context.ParameterInfo)))
    ...
    private static CustomBindingMethod DetectTryParseMethod(INamedTypeSymbol type)
    {
        foreach (var member in type.GetMembers("TryParse"))
        {
            if (member is not IMethodSymbol
                {
                    IsStatic: true, ReturnType.SpecialType: SpecialType.System_Boolean
                } method)
                continue;
            ...
            // First param must be string or ReadOnlySpan<char>
            var firstParamType = method.Parameters[0].Type;
            if (firstParamType.SpecialType != SpecialType.System_String &&
                !firstParamType.ToDisplayString().Contains("ReadOnlySpan<char>"))
                continue;

            // Last param must be out T
            var lastParam = method.Parameters[^1];
            if (lastParam.RefKind != RefKind.Out)
                continue;

            // Check for IFormatProvider overload
            if (method.Parameters.Length >= 3)
            {
                var middleParam = method.Parameters[1];
                var middleParamFqn = middleParam.Type.ToDisplayString();
                if (middleParamFqn.Contains("IFormatProvider"))
    ...
    private static bool IsFormFileType(ISymbol type, string typeFqn, ErrorOrContext context)
    {
        if (context.IFormFile is not null &&
            SymbolEqualityComparer.Default.Equals(type, context.IFormFile))
            return true;

        return typeFqn is WellKnownTypes.Fqn.IFormFile or "Microsoft.AspNetCore.Http.IFormFile" or "IFormFile";
    }

    private static bool IsFormFileCollectionType(ISymbol type, string typeFqn, ErrorOrContext context)
    {
        if (context.IFormFileCollection is not null &&
            SymbolEqualityComparer.Default.Equals(type, context.IFormFileCollection))
            return true;

        return typeFqn is WellKnownTypes.Fqn.IFormFileCollection or "Microsoft.AspNetCore.Http.IFormFileCollection"
        ...
        ITypeSymbol type)
    {
        ...
        else if (type is INamedTypeSymbol { IsGenericType: true } named)
        {
            ...
            if (origin.StartsWith("System.Collections.Generic.List") ||
                ...
                origin.StartsWith("System.Collections.Generic.ICollection") ||
                origin.StartsWith("System.Collections.Generic.HashSet"))
